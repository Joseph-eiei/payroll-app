...
function AttendanceReviewPage() {
  ...
  const fetchEmployees = async () => {
    try {
      const supRes = await axios.get('/api/employees/role/หัวหน้างาน');
      const empRes = await axios.get('/api/employees/role/พนักงาน');
      const techRes = await axios.get('/api/employees/role/ช่าง');

      setAvailableSupervisors(supRes.data);
      const employees = [...empRes.data, ...techRes.data];
      setAvailableEmployees(employees);

      const map = {};
      [...supRes.data, ...employees].forEach(e => {
